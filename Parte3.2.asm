#Parte 3.2 Laboratorio 2 Organizacion de computadores
#Programa que aproxima la funcion seno hiperbolico mediante serie de Taylor, da el resultado con dos decimales
#Por Florencia Corvalan Lillo
.data
	#Se definen los textos que se usaran eventualmente en el programa
	texto: .asciiz "Ingrese el numero del cual quiere calcular el seno hiperbolico: "
	...
	#Se define el orden de la serie de Taylor que debe ser un numero entero entre 0 y 5, ya que el factorial solo funciona hasta
	#el 12, y en esta serie se utiliza hasta el factorial de 2n + 1, donde n llega al orden determinado
	orden: .word 5
.text
	#Rutina que realiza la aproximacion de la funcion seno hiperbolico a traves de su serie de Taylor
	# (sumatoria de Xelevado(2n + 1) / (2n + 1)!, con n desde 0 hasta el valor de del orden)
	aproximacionSenoHiperbolico:
		
		#Se pide el texto para pedir el numero
		li $v0, 4
		la $a0, texto
		syscall
	...
	#Sub rutina que realiza el procedimiento necesario para aproximar la funcion seno hiperbolico a traves de la 
	#sumatoria de Xelevado(2n + 1) / (2n + 1)!, con n desde 0 hasta el valor de $s7 (el orden)
	calcularSenoHip:
		...
		#Si $s2 es mayor o igual a $s7 se salta nuevamente a calcularSenoHip
		bge $s2, $s7, calcularSenoHip
		################################################
		
		
		#Se retorna a donde se llamo a la sub rutina calcularSenoHip (no en los saltos, sino la primera vez)
		jr $ra
